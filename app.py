...
.extra-section{padding:10px 0 40px 0}
.section-head{background:#fff;border-radius:18px;padding:24px;margin-bottom:16px;box-shadow:0 10px 24px rgba(0,0,0,.04)}
.section-head h2{font-size:32px;color:#071b63;margin-bottom:6px}
.section-head p{color:#5b6d95;font-weight:700}
.op-cards{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}
.op-card{background:#fff;border-radius:18px;padding:20px;box-shadow:0 10px 24px rgba(0,0,0,.04);min-height:260px}
.op-card h3{color:#071b63;margin-bottom:8px}
.op-card p{color:#5b6d95;font-size:14px;margin-bottom:12px}
.op-item{border:1px solid #dbe4f2;border-radius:12px;padding:12px;margin-bottom:10px;background:#f8fbff}
.op-item b{display:block;color:#071b63;margin-bottom:5px}
.op-item small{color:#5b6d95;font-weight:700}
.btn-voltar{margin-top:18px;padding:12px 18px;border:0;border-radius:12px;background:#0a47ff;color:#fff;font-weight:900;cursor:pointer}
.btn-light{background:#fff;color:#071b63;border:1px solid #dbe4f2}
.fav-btn{display:inline-block;background:#ffc107;color:#081f4d;padding:7px 10px;border-radius:8px;text-decoration:none;font-size:12px;font-weight:900;margin:2px;border:0;cursor:pointer}
.remove-btn{display:inline-block;background:#ff3b30;color:#fff;padding:7px 10px;border-radius:8px;text-decoration:none;font-size:12px;font-weight:900;margin:2px;border:0;cursor:pointer}
.report-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:16px}
.report-card{background:#fff;border-radius:18px;padding:22px;box-shadow:0 10px 24px rgba(0,0,0,.04)}
.report-card h3{color:#5b6d95;font-size:14px;margin-bottom:8px}
.report-card strong{font-size:34px;color:#071b63}
.rel-list{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:10px}
.rel-chip{background:#f8fbff;border:1px solid #dbe4f2;border-radius:12px;padding:12px;font-weight:900;color:#071b63}
.actions-report{display:flex;gap:10px;flex-wrap:wrap}
@media(max-width:1200px){.op-cards,.report-grid{grid-template-columns:1fr}.rel-list{grid-template-columns:1fr 1fr}}
...
<div class="menu">
<a id="menuDashboard" class="active" onclick="mostrarDashboard()">🏠 Dashboard</a>
<a onclick="mostrarDashboard();document.getElementById('busca').focus()">🔎 Buscar Licitações</a>
<a id="menuOportunidades" onclick="mostrarOportunidades()">💎 Oportunidades</a>
<a id="menuFavoritos" onclick="mostrarFavoritos()">⭐ Favoritos</a>
<a id="menuRelatorios" onclick="mostrarRelatorios()">📊 Relatórios</a>
<a>⚙️ Configurações</a>
</div>
</div>
<div class="content">
...
<div id="oportunidadesSection" class="extra-section" style="display:none">
    <div class="section-head">
        <h2>💎 Oportunidades</h2>
        <p>Visão rápida das oportunidades encontradas na pesquisa atual.</p>
    </div>

    <div class="op-cards">
        <div class="op-card">
            <h3>🔥 Oportunidades em destaque</h3>
            <p>Licitações com objeto aderente à sua busca e edital disponível para análise.</p>
            <div id="oportunidadesDestaque"></div>
        </div>

        <div class="op-card">
            <h3>⏰ Encerram em breve</h3>
            <p>Licitações com menor prazo final de proposta.</p>
            <div id="oportunidadesPrazo"></div>
        </div>

        <div class="op-card">
            <h3>📍 Por UF</h3>
            <p>Estados com oportunidades retornadas na pesquisa.</p>
            <div id="oportunidadesUf"></div>
        </div>
    </div>

    <button class="btn-voltar" onclick="mostrarDashboard()">Voltar ao Dashboard</button>
</div>

<div id="favoritosSection" class="extra-section" style="display:none">
    <div class="section-head">
        <h2>⭐ Favoritos</h2>
        <p>Licitações salvas neste navegador para acompanhamento rápido.</p>
    </div>
    ...
    <div class="table-box">
        <table>
            ...
            <tbody id="listaFavoritos">
                <tr><td colspan="6">Nenhum favorito salvo.</td></tr>
            </tbody>
        </table>
    </div>
...
<div id="relatoriosSection" class="extra-section" style="display:none">
    <div class="section-head">
        <h2>📊 Relatórios</h2>
        <p>Resumo da pesquisa atual para conferência, impressão e apresentação.</p>
    </div>
    ...
    <div class="report-grid">
        ...
        <div class="report-card">
            <h3>Resultados na página</h3>
            <strong id="relPagina">0</strong>
        </div>
        <div class="report-card">
            <h3>Favoritos</h3>
            <strong id="relFavoritos">0</strong>
        </div>
    </div>

    <div class="table-box">
        <h3>Resumo por Modalidade</h3>
        <div id="relModalidades"></div>
    </div>
    ...
    <div class="actions-report">
        <button class="btn-voltar" onclick="window.print()">Gerar PDF</button>
        <button class="btn-voltar btn-light" onclick="mostrarDashboard()">Voltar ao Dashboard</button>
    </div>
</div>
...
let ultimosDados = {items:[], modalidades:{}, ufs:{}, total_pncp:0, pagina:1};
...
html+=`<tr><td>${i.orgao}</td><td>${i.objeto}</td><td>${i.uf}</td><td><span class='badge blue'>${i.modalidade}</span></td><td>${formatarData(i.inicio)}</td><td>${formatarData(i.fim)}</td><td><a class='action open' target='_blank' href='${i.link}'>Abrir</a><a class='action ai' href='${analisar}'>Analisar</a><button class='fav-btn' onclick='favoritar(${JSON.stringify(i)})'>⭐</button></td></tr>`;
})}
document.getElementById("resultado").innerHTML=html;
}

function setMenuAtivo(id){
["menuDashboard","menuOportunidades","menuFavoritos","menuRelatorios"].forEach(x=>{
...
document.getElementById("oportunidadesSection").style.display="none";
document.getElementById("favoritosSection").style.display="none";
document.getElementById("relatoriosSection").style.display="none";
setMenuAtivo("menuDashboard");
}

function mostrarOportunidades(){
document.getElementById("dashboardSection").style.display="none";
document.getElementById("oportunidadesSection").style.display="block";
document.getElementById("favoritosSection").style.display="none";
document.getElementById("relatoriosSection").style.display="none";
montarOportunidades();
setMenuAtivo("menuOportunidades");
}
...
montarRelatorios();
setMenuAtivo("menuRelatorios");
}

function getFavoritos(){
try{return JSON.parse(localStorage.getItem("rettFavoritos")||"[]")}catch(e){return[]}
}

function setFavoritos(lista){
localStorage.setItem("rettFavoritos",JSON.stringify(lista));
}

function favoritar(item){
let favs=getFavoritos();
let existe=favs.some(f=>f.link===item.link);
if(!existe){
favs.unshift(item);
setFavoritos(favs);
alert("Licitação adicionada aos favoritos.");
}else{
alert("Essa licitação já está nos favoritos.");
}
}

function removerFavorito(link){
let favs=getFavoritos().filter(f=>f.link!==link);
setFavoritos(favs);
renderFavoritos();
}
...
let favs=getFavoritos();
let tbody=document.getElementById("listaFavoritos");
if(!tbody)return;

if(favs.length===0){
tbody.innerHTML="<tr><td colspan='6'>Nenhum favorito salvo.</td></tr>";
return;
}
...
<td>${i.objeto}</td>
<td>${i.uf}</td>
<td><span class='badge blue'>${i.modalidade}</span></td>
<td>${formatarData(i.fim)}</td>
<td>
<a class='action open' target='_blank' href='${i.link}'>Abrir</a>
<a class='action ai' href='${analisar}'>Analisar</a>
<button class='remove-btn' onclick='removerFavorito(${JSON.stringify(i.link)})'>Remover</button>
</td>
</tr>`;
});
...
let itens=ultimosDados.items||[];

let destaque=itens.slice(0,5);
let prazo=[...itens].sort((a,b)=>String(a.fim||"").localeCompare(String(b.fim||""))).slice(0,5);

let porUf={};
itens.forEach(i=>{porUf[i.uf]=(porUf[i.uf]||0)+1});

document.getElementById("oportunidadesDestaque").innerHTML=cardsOportunidade(destaque);
document.getElementById("oportunidadesPrazo").innerHTML=cardsOportunidade(prazo);

let ufHtml="";
Object.entries(porUf).sort((a,b)=>b[1]-a[1]).forEach(([uf,total])=>{
ufHtml+=`<div class="op-item"><b>${uf}</b><small>${total} oportunidade(s)</small></div>`;
});
document.getElementById("oportunidadesUf").innerHTML=ufHtml||"<p>Nenhuma UF encontrada.</p>";
}

function cardsOportunidade(lista){
if(!lista||lista.length===0)return "<p>Nenhuma oportunidade encontrada na pesquisa atual.</p>";
let html="";
lista.forEach(i=>{
html+=`<div class="op-item">
<b>${i.orgao}</b>
<small>${i.uf} | ${i.modalidade} | Fim: ${formatarData(i.fim)}</small>
<p style="margin-top:6px;color:#233653;font-size:13px">${i.objeto}</p>
</div>`;
});
return html;
}
...
let dados=ultimosDados||{};
let favs=getFavoritos();

document.getElementById("relTotal").innerText=dados.total_pncp||0;
document.getElementById("relPagina").innerText=(dados.items||[]).length;
document.getElementById("relFavoritos").innerText=favs.length;

document.getElementById("relModalidades").innerHTML=chipsRelatorio(dados.modalidades||{});
document.getElementById("relUfs").innerHTML=chipsRelatorio(dados.ufs||{});
}

function chipsRelatorio(obj){
let entries=Object.entries(obj||{}).filter(([k,v])=>Number(v)>0).sort((a,b)=>b[1]-a[1]).slice(0,20);
if(entries.length===0)return "<p>Nenhum dado disponível.</p>";
let html="<div class='rel-list'>";
entries.forEach(([k,v])=>{html+=`<div class='rel-chip'>${k}: ${v}</div>`});
html+="</div>";
return html;
}
