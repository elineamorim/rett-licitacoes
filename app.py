...
def extrair_itens_reais(texto, palavras_inicio, palavras_fim, limite=120):
    ...
    Evita criar lista genérica e mantém o conteúdo conforme aparece no edital.
    """
    linhas = texto.split("\n")
    capturando = False
    encontrados = []
    ...
    for linha in linhas:
        ...
        if capturando:
            if len(encontrados) > 3 and any(p in l_lower for p in palavras_fim):
                break
            ...
            # Mantém itens numerados, alíneas e bullets quando estiverem na seção certa
            if re.match(r'^(\d+(\.\d+)*[\.\)]?|\-|\•|[a-z]\))\s+', l_lower):
                encontrados.append(l)

            # Mantém linhas com documentos/exigências específicas
            elif any(k in l_lower for k in termos_documentos):
                encontrados.append(l)

        if len(encontrados) >= limite:
            break

    # Limpeza: remove duplicados e linhas muito genéricas
    final = []
    genericas = [
    ...
    for item in encontrados:
        ...
        item_lower = item_limpo.lower()

        if item_lower in genericas:
            continue

        if item_limpo not in final:
