...
def extrair_itens_reais(texto, palavras_inicio, palavras_fim, limite=160):
    """
    Extrai itens reais do edital preservando o texto completo.

    Correção principal:
    - NÃO corta a frase quando ela continua na linha seguinte.
    - Só fecha um item quando encontra outro item numerado/alínea/bullet.
    - Evita usar palavras_fim para cortar no meio da frase.
    - Não inventa conteúdo: apenas reorganiza o texto extraído do edital.
    """
    linhas = texto.split("\n")
    capturando = False
    encontrados = []
    ...
    termos_relevantes = [
        "certidão", "certidao", "certificado", "regularidade",
        "balanço", "balanco", "demonstração", "demonstracao",
        "contábil", "contabil", "índice", "indice",
        "atestado", "capacidade", "fgts", "cndt", "cnpj",
        "sicaf", "caf", "declaração", "declaracao",
        "procuração", "procuracao", "garantia", "seguro-garantia",
        "seguro garantia", "fiança", "fianca", "caução", "caucao",
        "vistoria", "sei", "sistema eletrônico", "sistema eletronico",
        "assinatura", "contrato", "prazo", "termo de referência",
        "termo de referencia", "obrigações", "obrigacoes",
        "execução", "execucao", "empresário individual",
        "empresario individual", "microempreendedor", "mei",
        "registro público", "registro publico", "documentos pessoais",
        "sócios", "socios", "rg", "cpf", "cnh", "fazenda",
        "tributos", "trabalhista", "falência", "falencia",
        "recuperação judicial", "recuperacao judicial", "veículo",
        "veiculo", "crlv", "licenciamento", "motorista",
        "requerimento", "anexo", "credenciado", "convocação",
        "convocacao", "classificação", "classificacao"
    ]
    ...
    def linha_de_rodape(linha_lower):
        if any(i in linha_lower for i in ignorar_linha):
            return True

        # Remove linhas que parecem apenas endereço/rodapé quando entram no meio do item
        if re.search(r'\(\d{2}\)\s*\d{4,5}[-\s]?\d{4}', linha_lower):
            return True
    ...
    def salvar_item():
        ...
        if item_lower in genericos:
            item_atual = ""
            return

        if any(t in item_lower for t in termos_relevantes) or re.match(r'^\d+(\.\d+)*', item_lower):
    ...
    for linha in linhas:
        ...
        if padrao_inicio_item.match(l_lower):
            ...
            # Aqui está a correção: junta tudo até aparecer outro número/alínea.
            if item_atual:
                item_atual += " " + l
            else:
                # Começa item sem número apenas se a linha for claramente relevante
                if any(t in l_lower for t in termos_relevantes):
                    item_atual = l
    ...
    for item in encontrados:
        ...
        # Remove pedaços claramente quebrados/soltos
        if len(item) < 12:
            continue
...
def limpar_item_extraido(item):
    """
    Limpa o texto do edital sem inventar conteúdo.
    Mantém o conteúdo real, apenas remove espaços duplicados e quebras ruins.
    """
...
def item_valido_edital(item):
    """
    Evita título solto ou linha quebrada inútil.
    Não cria informação nova.
    """
    item = limpar_item_extraido(item)
    ...
    ruins = [
        ...
        "da fase de habilitacao",
        "habilitação",
        "habilitacao",
    ]

    if item.lower() in ruins:
        return False
...
def reconstruir_itens_quebrados(itens):
    """
    Junta frases quebradas pelo PDF.
    Exemplo:
    6.3.2 prova de regularidade para com a fazenda federal, mediante a
    apresentação de certidão conjunta...
    vira um único item completo.
    """
    if not itens:
        return []
    ...
    padrao_inicio = re.compile(r"^(\d+(\.\d+)*[\.)]?|[a-z]\)|-|•)\s+")

    for item in itens:
        linha = limpar_item_extraido(item)

        if not linha:
            continue

        if padrao_inicio.match(linha.lower()):
            if atual:
    ...
    for item in resultado:
        ...
        if chave not in vistos and item_valido_edital(item):
            final.append(item)
            vistos.add(chave)
...
def categorizar_documentos_habilitacao(itens):
    """
    Organiza a documentação exigida por categoria, mas APENAS com itens encontrados no edital.
    Não adiciona nenhum documento padrão.
    """
    itens = reconstruir_itens_quebrados(itens)
    ...
    categorias = {
        ...
        "Outras exigências de habilitação": [],
    }

    for item in itens:
        item = limpar_item_extraido(item)

        if not item_valido_edital(item):
            continue
        ...
        if any(p in t for p in [
            ...
            categorias["Habilitação jurídica"].append(item)
            ...
            categorias["Regularidade fiscal e trabalhista"].append(item)
            ...
            categorias["Qualificação econômico-financeira"].append(item)
            ...
            categorias["Qualificação técnica"].append(item)
            ...
            categorias["Declarações e anexos"].append(item)

        else:
            categorias["Outras exigências de habilitação"].append(item)

    return {k: v for k, v in categorias.items() if v}


def categorizar_exigencias_contratacao(itens):
    """
    Organiza as exigências para contratação, mas APENAS com itens encontrados no edital.
    Não presume garantia, prazo, vistoria ou obrigação se não aparecer no texto extraído.
    """
    itens = reconstruir_itens_quebrados(itens)
    ...
    categorias = {
        ...
        "Execução do objeto": [],
        "Obrigações da contratada": [],
        "Vistoria / ciência das condições": [],
        "Prazos e condições": [],
        "Outras exigências para contratação": [],
    }
    ...
    for item in itens:
        ...
        if any(p in t for p in [
            ...
            categorias["Assinatura e formalização"].append(item)
            ...
            categorias["Garantia contratual"].append(item)
            ...
            categorias["Execução do objeto"].append(item)
            ...
            categorias["Obrigações da contratada"].append(item)
            ...
            categorias["Vistoria / ciência das condições"].append(item)
            ...
            categorias["Prazos e condições"].append(item)

        else:
            categorias["Outras exigências para contratação"].append(item)

    return {k: v for k, v in categorias.items() if v}


def html_categorias(categorias):
    """
    Monta HTML organizado por categoria.
    Só exibe categorias que possuem itens reais extraídos do edital.
    """
    if not categorias:
        return ""
...
def css_relatorio():
    return """
    <style>
    body{font-family:Arial;background:#f4f7fc;padding:35px;color:#081f4d}
    .page{background:white;max-width:1250px;margin:auto;padding:35px;border-radius:18px}
    h1{color:#061f63;font-size:32px;margin-bottom:10px}
    .subinfo{color:#5b6d95;line-height:1.6;margin-bottom:24px}
    .bloco{background:#f7f9fc;border-left:5px solid #0a47ff;padding:18px;border-radius:12px;margin-bottom:18px}
    .bloco h3{margin-bottom:12px;color:#0a2a7a;font-size:21px}
    .bloco p,.bloco li{font-size:15px;line-height:1.65;color:#24344d}
    .bloco ul{padding-left:22px}
    .categoria{background:#fff;border:1px solid #dbe4f2;border-radius:12px;padding:14px;margin-top:12px}
    .categoria h4{font-size:17px;color:#0a2a7a;margin-bottom:8px}
    .categoria li{margin-bottom:5px}
    .btn{padding:9px 14px;border:0;border-radius:8px;background:#0a47ff;color:white;font-weight:bold;cursor:pointer;margin-right:8px;text-decoration:none;display:inline-block}
    .btn2{padding:9px 14px;border:1px solid #ccc;border-radius:8px;background:white;color:#081f4d;font-weight:bold;cursor:pointer;text-decoration:none;display:inline-block}
    .campo{border-bottom:1px solid #333;display:inline-block;min-width:280px;height:22px}
    .texto-edital{white-space:pre-wrap;background:#ffffff;border:1px solid #dbe4f2;padding:22px;border-radius:12px;font-family:Arial, Helvetica, sans-serif;font-size:15px;line-height:1.9;overflow:auto;color:#1f2d3d;text-align:left;word-break:break-word}
    @media print{button,.btn,.btn2{display:none}.page{box-shadow:none;margin:0;max-width:none}}
    </style>
    """
...
def analisar(link: str = "", objeto: str = "", orgao: str = "", modalidade: str = "", valor: str = ""):
    ...
    bloco_docs = ""
    if docs:
        docs_categorizados = categorizar_documentos_habilitacao(docs)
        html_docs = html_categorias(docs_categorizados)

        if html_docs:
            bloco_docs = f"""
            <div class="bloco">
                <h3>📄 Documentação Exigida para Habilitação</h3>
                <p class="subinfo">Itens localizados no edital, organizados por categoria, com frases continuadas reunidas automaticamente. O sistema não adiciona documentos que não estejam no texto analisado.</p>
                {html_docs}
            </div>
            """
    ...
    bloco_exig = ""
    if exig:
        exig_categorizadas = categorizar_exigencias_contratacao(exig)
        html_exig = html_categorias(exig_categorizadas)

        if html_exig:
            bloco_exig = f"""
            <div class="bloco">
                <h3>🛠️ Exigências para Contratação</h3>
                <p class="subinfo">Exigências localizadas no edital, organizadas por categoria, com frases continuadas reunidas automaticamente. O sistema não presume exigências.</p>
                {html_exig}
            </div>
            """
    ...
    <div class="bloco">
        ...
        <ul>
            ...
            <li><b>Valor estimado:</b> {escape(valor) if valor else "Não informado."}</li>
