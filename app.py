...
def extrair_itens_reais(texto, palavras_inicio, palavras_fim, limite=120):
    ...
    Corrige o problema de PDF quebrar frases em várias linhas.

    Regra:
    - Encontra a seção pelo termo inicial.
    - Começa um item quando encontra numeração/alínea/bullet.
    - Continua juntando as linhas seguintes até aparecer outro item numerado.
    - Só mostra o que está no edital.
    """
    linhas = texto.split("\n")
    capturando = False
    encontrados = []
    ...
    def salvar_item():
        ...
        if any(i in item_lower for i in ignorar):
            item_atual = ""
            return

        # Mantém o item se ele estiver dentro da seção e tiver conteúdo relevante
        # ou se for item numerado da seção de habilitação/contratação.
    ...
    for linha in linhas:
        ...
        if capturando:
            # Para somente depois que já tiver coletado algo, para não parar no primeiro subtítulo
            if len(encontrados) > 2 and any(p in l_lower for p in palavras_fim):
                salvar_item()
                break
            ...
            if padrao_inicio_item.match(l_lower):
                ...
                # linha complementar do item anterior
                if item_atual:
                    item_atual += " " + l
                else:
                    # permite começar por linha sem número quando ela é claramente relevante
